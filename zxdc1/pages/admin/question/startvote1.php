...
if($khtype==1){
	...
	//创建中层领导班子互评表（校领导评中层班子）
	 ...
	 $query_rsVoteRecord="CREATE TABLE ".$tableName." (
		  ...
		  `ZHKH` varchar(255) CHARACTER SET utf8 DEFAULT NULL,
		  PRIMARY KEY (`id`) USING BTREE,
		  KEY `deptID` (`DeptID`) USING BTREE,
		  KEY `UserID` (`UserID`) USING BTREE,
		  CONSTRAINT `deptID` FOREIGN KEY (`DeptID`) REFERENCES `deptinfo` (`DeptID`) ON DELETE CASCADE ON UPDATE CASCADE,
		  CONSTRAINT `zc_ldbzkhinfo_ibfk_1` FOREIGN KEY (`UserID`) REFERENCES `userinfo` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE
	 ...
	 $query_rsVoteRecord="CREATE TABLE ".$tableName."(`id` int(11) NOT NULL AUTO_INCREMENT,
		  ...
		  INDEX `depid`(`DeptID`) USING BTREE,
		  INDEX `bpuser`(`BPUserID`) USING BTREE
	...
	$query_rsVoteRecord="CREATE TABLE ".$tableName." (
		  ...
		  CONSTRAINT `bpuser1` FOREIGN KEY (`BPUserID`) REFERENCES `userinfo` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE,
		  CONSTRAINT `depid1` FOREIGN KEY (`DeptID`) REFERENCES `deptinfo` (`DeptID`) ON DELETE CASCADE ON UPDATE CASCADE,
		  CONSTRAINT `userid1` FOREIGN KEY (`UserID`) REFERENCES `userinfo` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE
	...
	//创建群众评本部门领导班子表
	 $tableName="qz_ldbzkhinfo_".$recordcode;
		  ...
		  CONSTRAINT `depid2` FOREIGN KEY (`DeptID`) REFERENCES `deptinfo` (`DeptID`) ON DELETE CASCADE ON UPDATE CASCADE,
		  CONSTRAINT `userid2` FOREIGN KEY (`UserID`) REFERENCES `userinfo` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE
