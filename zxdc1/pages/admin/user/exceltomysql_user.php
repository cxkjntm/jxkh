...
$sql = "INSERT INTO UserInfo_old(Account, UserName, Passwd, Photo, DeptID, Rank,IsEvaluated, IsPj, IsBanned) VALUES  ";
// 获取一行的数据
for ($row = 2; $row <= $highestRow; $row++){
	...
	$username = $objPHPExcel->getActiveSheet()->getCell("D".$row)->getValue();//获取D列的值
	$deptid = $objPHPExcel->getActiveSheet()->getCell("A".$row)->getValue();//获取A列的值
   	$level = $objPHPExcel->getActiveSheet()->getCell("E".$row)->getValue();//获取E列的值
   ...
   //echo "name=".$name.$sex.$age.$rank;
	...
	//echo "<br>";
	if ($row==$highestRow)
		$sql1=$sql1."('$account','$username','$password','$photo','$deptid','$level','$isevaluted','$ispj','$isbanned')".";";
    else
		$sql1=$sql1."('$account','$username','$password','$photo','$deptid','$level','$isevaluted','$ispj','$isbanned')".",";
	//echo $sql1;	
