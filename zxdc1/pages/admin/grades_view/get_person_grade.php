...
<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend align="center"><b>中层领导成员当前分数</b></legend>
</fieldset>
...
$tableName="grades_".$rs['RecordCode'];
...
$result =mysql_query($sql, $connjxkh);
    while($row=mysql_fetch_row($result)){          //While()的作用是将指针向后移动，将下一行的数据交给$row
          echo "<tr>";
          //print_r($row);
          $sqlinfo = "select * from userinformation where UserID = ".$row[1];
          $rs2 = mysql_fetch_assoc(mysql_query($sqlinfo, $connjxkh));
          //print_r($rs2);
          echo "<td>".$rs2['UserName']."</td><td>".$rs2['DeptName']."</td>";
          switch($rs2['LevelID']){
            case 1 : echo "<td> 校级领导</td>";break;
            case 2 : echo "<td> 正处级干部</td>";break;
            case 3 : echo "<td> 副处级干部</td>";break;
          }
          echo "<td>".$row[2]."</td></tr>";
    }
?>
